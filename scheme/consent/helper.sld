;;; Portable Consent Scheme helper artifact records.
...
(define-library (consent helper)
  ...
  (begin
    ...
    (define-record-type <consent-helper-store>
    ...
    (define (normalize-scope scope)
      ...
      (let ((normalized (if (eq? scope 'project) 'project-private scope)))
        (if (member-equal? normalized consent-helper-scopes)
            normalized
    ...
    (define (helper-save! store scope library-name forms source)
      ...
      #((parameters
         ...
         (forms
          (type any)
          (description "Helper source forms as Scheme-readable data."))
         (source
          (type any)
          (description
           ("Source metadata describing where the helper came from."))))
        (returns
         (type any)
    ...
    (define (artifact-save! store scope name datum source)
      ...
      #((parameters
         ...
         (datum
          (type any)
          (description "Artifact payload as Scheme-readable data."))
         (source
          (type any)
          (description
           ("Source metadata describing where the artifact came from."))))
        (returns
         (type any)
