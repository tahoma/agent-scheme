;;; SRFI 42 eager comprehensions support for stdlib.
;; SPDX-License-Identifier: MIT
;; SPDX-FileCopyrightText: 2003, 2007 Sebastian Egner
;; SPDX-FileCopyrightText: 2026 Tahoma Toelkes
;;;
;;; Vendored from the official SRFI 42 reference implementation at
;;; <https://github.com/scheme-requests-for-implementation/srfi-42>,
;;; revision 6a4c4aeb4cb61c6514776144fdfc5fe0d738a548, file ec.scm.
;;; Local patches wrap the implementation in an R7RS define-library form,
;;; import R7RS read support for the default :port generator, and expose
;;; (srfi 42), (srfi srfi-42), (srfi :42), and
;;; (srfi :42 eager-comprehensions) as manifest aliases.

(define-library (stdlib eager-comprehensions)
  (export
   do-ec
   list-ec append-ec string-ec string-append-ec vector-ec
   vector-of-length-ec sum-ec product-ec min-ec max-ec
   first-ec last-ec fold-ec fold3-ec any?-ec every?-ec
   : :list :string :vector :range :real-range :char-range :port
   :do :let :parallel :while :until :integers
...
;; Bind VAR to EXPRESSION inside a comprehension.
(define-syntax :let
  (syntax-rules (index)
...
;; Run multiple generators in lockstep.
...
;; Run a generator while TEST remains true.
(define-syntax :while
  (syntax-rules ()
...
;; Preserve inner bindings while adding a :while guard.
...
;; Run a generator until TEST becomes true.
(define-syntax :until
  (syntax-rules ()
    ((:until cc (g arg1 arg ...) test)
     (g (:until-1 cc test) arg1 arg ...) )))

;; Add an until guard to a decorated :do generator.
...
(define (ec-:vector-filter vecs)
  "Return non-empty vectors from VECS for the :vector generator."
  (if (null? vecs)
      '()
...
;; Iterate over exact integer ranges.
(define-syntax :range
  (syntax-rules (index)
...
(define (dispatch-union d1 d2)
  "Return a dispatcher that tries D1 and D2 and rejects conflicts."
  #((parameters
     (d1 (type procedure)
      (description "First SRFI 42 dispatch procedure."))
     (d2 (type procedure)
      (description "Second SRFI 42 dispatch procedure.")))
    (returns (type procedure)
     (description "A dispatch procedure combining D1 and D2."))
    (effects allocation procedure-call error))
  (lambda (args)
...
(define (make-initial-:-dispatch)
  "Return the initial SRFI 42 dispatch procedure for :."
  #((parameters)
    (returns (type procedure)
     (description "The default dispatcher used by the : generator."))
    ...
    (case (length args)
      ...
      ((2) (let ((a1 (car args)) (a2 (cadr args)))
             (cond
              ...
              (else
               #f ))))
      ((3) (let ((a1 (car args)) (a2 (cadr args))
                 (a3 (car (cdr (cdr args)))))
             (cond
...
;; Current dispatch procedure for the default : generator.
(define :-dispatch
  (make-initial-:-dispatch) )

(define (:-dispatch-ref)
  "Return the current SRFI 42 dispatch procedure."
  #((parameters)
    (returns (type procedure)
     (description "The dispatch procedure currently used by :."))
    (effects state-read))
  :-dispatch )

(define (:-dispatch-set! dispatch)
  "Replace the current SRFI 42 dispatch procedure with DISPATCH."
  #((parameters
     (dispatch (type procedure)
      (description "New dispatch procedure used by :.")))
    (returns . ("An unspecified value after replacing the dispatcher."))
    (effects state-write error))
  (if (not (procedure? dispatch))
      (error "not a procedure" dispatch) )
  (set! :-dispatch dispatch) )

;; Dispatch to the current type-directed generator.
(define-syntax :
  (syntax-rules (index)
...
;; Collect eager-comprehension characters into a string.
...
;; Collect eager-comprehension values into a vector.
...
;; Sum eager-comprehension values.
(define-syntax sum-ec
  (syntax-rules ()
    ((sum-ec etc1 etc ...)
     (fold-ec (+) etc1 etc ... +) )))

;; Multiply eager-comprehension values.
(define-syntax product-ec
  (syntax-rules ()
    ((product-ec etc1 etc ...)
     (fold-ec (*) etc1 etc ... *) )))

;; Return the minimum eager-comprehension value.
(define-syntax min-ec
  (syntax-rules ()
    ((min-ec etc1 etc ...)
     (fold3-ec (min) etc1 etc ... min min) )))

;; Return the maximum eager-comprehension value.
(define-syntax max-ec
  (syntax-rules ()
    ((max-ec etc1 etc ...)
     (fold3-ec (max) etc1 etc ... max max) )))

;; Return the last eager-comprehension value or DEFAULT.
...
;; Return true when every eager-comprehension value is true.
