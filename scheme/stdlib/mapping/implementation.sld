;;; Provider-neutral SRFI 146 ordered mapping implementation.
...
(define-library (stdlib mapping implementation)
  ...
  (import (scheme base)
          ...
          (stdlib rbtree))
  (begin
    ;; Ordered providers isolate storage mechanics from Mapping algorithms.
    (define-record-type <mapping-provider>
      (make-ordered-mapping-provider
       name empty search for-each fold fold/reverse generator
       predecessor successor split catenate map/monotone)
      mapping-provider?
      (name mapping-provider-name)
      (empty mapping-provider-empty)
      (search mapping-provider-search)
      (for-each mapping-provider-for-each)
      (fold mapping-provider-fold)
      (fold/reverse mapping-provider-fold/reverse)
      (generator mapping-provider-generator)
      (predecessor mapping-provider-predecessor)
      (successor mapping-provider-successor)
      (split mapping-provider-split)
      (catenate mapping-provider-catenate)
      (map/monotone mapping-provider-map/monotone))

    (define (mapping-provider=? left right)
      "Return #t when LEFT and RIGHT identify the same storage provider."
      #((parameters
    ...
    ;; Default ordered Mapping provider backed by red-black trees.
    (define red-black-mapping-provider
      (make-ordered-mapping-provider
       'red-black
       (lambda (comparator) (make-tree))
       tree-search
       tree-for-each
       tree-fold
       tree-fold/reverse
       ...
       tree-key-successor
       tree-split
       (lambda (comparator left key value right)
         (tree-catenate left key value right))
       (lambda (procedure comparator root)
         (tree-map procedure root))))

    ;; Ordered mappings pair a comparator, provider, and provider-owned root.
    (define-record-type <mapping>
      (%make-mapping comparator provider root)
      raw-mapping?
      (comparator raw-mapping-key-comparator)
      (provider mapping-storage-provider)
      (root mapping-root))
    ...
    (define (make-empty-mapping/provider comparator provider)
      "Return an empty mapping using COMPARATOR and PROVIDER."
      (assume (comparator? comparator))
      (assume (mapping-provider? provider))
      (%make-mapping comparator
                     provider
                     ((mapping-provider-empty provider) comparator)))

    (define (make-empty-mapping comparator)
      "Return an empty red-black ordered mapping using COMPARATOR."
      (make-empty-mapping/provider comparator red-black-mapping-provider))

    (define (mapping-with-provider provider comparator . args)
      "Return a mapping backed by PROVIDER and initialized from ARGS."
      #((parameters
         (provider (type mapping-provider) (description "Storage provider."))
         (comparator (type comparator) (description "Key comparator."))
         (args (type list) (description "Alternating keys and values.")))
        (returns (type mapping) (description "Initialized mapping."))
        (effects allocation error procedure-call))
      (mapping-unfold-with-provider
       provider
       null?
       (lambda (args) (values (car args) (cadr args)))
       cddr
       args
       comparator))
    ...
    (define (mapping-unfold-with-provider
             provider stop? mapper successor seed comparator)
      "Return a mapping unfolded into PROVIDER from SEED."
      #((parameters
         (provider (type mapping-provider) (description "Storage provider."))
         (stop? (type procedure) (description "Seed termination predicate."))
         (mapper (type procedure) (description "Seed key/value producer."))
         (successor (type procedure) (description "Seed successor."))
         (seed (type any) (description "Initial unfold seed."))
         (comparator (type comparator) (description "Key comparator.")))
        (returns (type mapping) (description "Unfolded mapping."))
        (effects allocation error procedure-call))
      (assume (mapping-provider? provider))
      ...
      (assume (comparator? comparator))
      (let loop ((mapping (make-empty-mapping/provider comparator provider))
                 (seed seed))
        (if (stop? seed)
            mapping
            (receive (key value)
    ...
    (define (mapping-unfold stop? mapper successor seed comparator)
      ...
      (assume (procedure? successor))
      (assume (comparator? comparator))
      (mapping-unfold-with-provider red-black-mapping-provider
                                    stop?
                                    mapper
                                    successor
                                    seed
      ...
      (call/cc
       (lambda (return)
         (let* ((comparator (mapping-key-comparator mapping))
                (provider (mapping-storage-provider mapping)))
           (let-values
               (((root obj)
                 ((mapping-provider-search provider)
                  comparator
                  ...
                  (lambda (insert ignore)
                    (failure
                     ...
                     (lambda (obj)
                       (return mapping obj))))
                  success)))
             (values (%make-mapping comparator provider root)
                     obj))))))
    ...
    (define (mapping-map proc comparator mapping)
      ...
      (mapping-fold (lambda (key value mapping)
                    ...
                    (make-empty-mapping/provider
                     comparator
                     (mapping-storage-provider mapping))
                    mapping))
    ...
    (define (mapping-for-each proc mapping)
      ...
      (assume (procedure? proc))
      (assume (mapping? mapping))
      ((mapping-provider-for-each (mapping-storage-provider mapping))
       proc
       (mapping-root mapping)))
    ...
    (define (mapping-fold proc acc mapping)
      ...
      (assume (procedure? proc))
      (assume (mapping? mapping))
      ((mapping-provider-fold (mapping-storage-provider mapping))
       proc
       acc
       (mapping-root mapping)))
    ...
    (define (alist->mapping-with-provider provider comparator alist)
      "Return a PROVIDER-backed ordered mapping containing ALIST."
      #((parameters
         (provider (type mapping-provider) (description "Storage provider."))
         (comparator (type comparator) (description "Key comparator."))
         (alist (type list) (description "Associations to add.")))
        (returns (type mapping) (description "Converted mapping."))
        (effects allocation error procedure-call))
      (assume (mapping-provider? provider))
      (assume (comparator? comparator))
      (assume (list? alist))
    ...
    (define (%mapping<=? comparator mapping1 mapping2)
      ...
      (let ((less? (comparator-ordering-predicate
            ...
            (gen1 ((mapping-provider-generator
                    (mapping-storage-provider mapping1))
                   (mapping-root mapping1)))
            (gen2 ((mapping-provider-generator
                    (mapping-storage-provider mapping2))
                   (mapping-root mapping2))))
        (let loop ((item1 (gen1))
                   (item2 (gen2)))
          (cond
           ((eof-object? item1) #t)
    ...
    (define (mapping-key-predecessor mapping obj failure)
      ...
      (assume (mapping? mapping))
      (assume (procedure? failure))
      ((mapping-provider-predecessor (mapping-storage-provider mapping))
       (mapping-key-comparator mapping)
       (mapping-root mapping)
       obj
       failure))
    ...
    (define (mapping-key-successor mapping obj failure)
      ...
      (assume (mapping? mapping))
      (assume (procedure? failure))
      ((mapping-provider-successor (mapping-storage-provider mapping))
       (mapping-key-comparator mapping)
       (mapping-root mapping)
       obj
       failure))
    ...
    (define (mapping-range= mapping obj)
      ...
      (let ((comparator (mapping-key-comparator mapping))
        ...
        (receive (tree< tree<= tree= tree>= tree>)
            ((mapping-provider-split provider)
             comparator (mapping-root mapping) obj)
    ...
    (define (mapping-split mapping obj)
      ...
      (let ((comparator (mapping-key-comparator mapping))
        ...
        (receive (tree< tree<= tree= tree>= tree>)
            ((mapping-provider-split provider)
             comparator (mapping-root mapping) obj)
          (values (%make-mapping comparator provider tree<)
                  (%make-mapping comparator provider tree<=)
                  (%make-mapping comparator provider tree=)
                  (%make-mapping comparator provider tree>=)
                  (%make-mapping comparator provider tree>)))))
    ...
    (define (mapping-catenate comparator mapping1 pivot-key pivot-value mapping2)
      ...
      (assume (mapping? mapping1))
      (assume (mapping? mapping2))
      (let ((provider (mapping-storage-provider mapping1)))
        (if (mapping-provider=? provider
                                (mapping-storage-provider mapping2))
            (%make-mapping
             comparator
             provider
             ((mapping-provider-catenate provider)
              comparator
              (mapping-root mapping1)
              pivot-key
              pivot-value
              (mapping-root mapping2)))
            (mapping-fold
             (lambda (key value result) (mapping-set result key value))
             (mapping-set
              (mapping-fold
               (lambda (key value result) (mapping-set result key value))
               (make-empty-mapping/provider comparator provider)
               mapping1)
              pivot-key
              pivot-value)
             mapping2))))
    ...
    (define (mapping-map/monotone proc comparator mapping)
      ...
      (assume (comparator? comparator))
      (assume (mapping? mapping))
      (let ((provider (mapping-storage-provider mapping)))
        (%make-mapping
         comparator
         provider
         ((mapping-provider-map/monotone provider)
          proc comparator (mapping-root mapping)))))
    ...
    (define (mapping-fold/reverse proc acc mapping)
      ...
      (assume (procedure? proc))
      (assume (mapping? mapping))
      ((mapping-provider-fold/reverse (mapping-storage-provider mapping))
       proc
       acc
       (mapping-root mapping)))
