...
- `(agent-scheme-version)` returns the canonical Agent Scheme runtime version
  ...
  `(agent-scheme-version 0 15 1)`: primary version `0`, secondary version
  `15` for the roadmap chunk, and tertiary version `1` for the issue's
  position in that chunk. Strings such as `0.15.1` are derived presentation,
...
- `(agent-scheme-version)` reports the roadmap-derived runtime version as
  `(agent-scheme-version 0 15 1)`, and the Emacs host-adapter fixture points
