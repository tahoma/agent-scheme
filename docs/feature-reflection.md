...
- `(agent-scheme-version)` returns the canonical Agent Scheme runtime version
  ...
  `(agent-scheme-version 0 14 12)`: primary version `0`, secondary version
  `14` for the roadmap chunk, and tertiary version `12` for the issue's
  position in that chunk. Strings such as `0.14.12` are derived presentation,
...
- `(agent-scheme-version)` reports the roadmap-derived runtime version as
  `(agent-scheme-version 0 14 12)`, and the Emacs host-adapter fixture points
