...
- **Tracked file, commit message, or branch name** — remove the branding from
  the offending file, amend the commit message, or rename the branch.
- **Commit identity** — a commit was authored or committed under a vendor-bot
  identity (the author email is what GitHub attributes the commit to). Re-author
  the range under your own identity, for example
  `git rebase <base> --exec 'git commit --amend --reset-author --no-edit'`, then
  force-push. Confirm your `git config user.name`/`user.email` are set first.
- **PR title or body** — this is most often a "Generated by …" trailer that the
  pull-request-creation tooling appends *after* a clean body is supplied. Remove
  it by editing the PR description (an explicit pull-request update, or the PR
  edit UI) and re-run the check. An explicit update is not re-injected; only PR
  creation adds the trailer, so the edit sticks. Re-read the description
  afterward to confirm rather than assuming the edit held.

The gate prints this same PR-title/body remediation hint in its failure output,
so the fix travels with the red check.
