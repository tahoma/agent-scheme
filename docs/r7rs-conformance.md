...
| 6.12-6.14 eval, I/O, and system interface | Explicit eval environments, in-memory textual and binary ports, read/write round trips, EOF objects, default host port policy, file/process/time/repl policy gates | Host-backed ports and system effects remain policy-gated; oracle eligibility marks cases that reference implementations cannot exercise under the same policy model. |
