...
The schema v2 corpus contains 167 cases: 137 `eval`, one `eval-result`, 28
`read`, and one `read-all` case. Expectations comprise 128 single values, two
multiple-value records, 36 conditions, and one result record.
...
| 6.1-6.9 core datums and numbers | Equivalence, booleans, pairs/lists, symbols, owned Unicode-scalar characters, strings, vectors, bytevectors, exact/inexact numeric operations, radix I/O, rationalization, infinities, NaNs, complex numbers | Character semantics use the documented bootstrap Unicode profile; generated full-table breadth remains tracked separately. `eqv?` edge cases and numeric branch cuts remain intentionally representative rather than exhaustive. |
...
| --- | --- | --- | --- | --- |
| Reader syntax | Comments, case directives, booleans, numbers, characters, strings, symbols, lists, dotted pairs, abbreviations, vectors, bytevectors, datum comments, datum labels | `implemented` | `reader-boolean-literals`, `reader-long-boolean-literal`, `reader-bytevector-literal`, `reader-bytevector-byte-range-error`, `reader-character-literal`, `reader-character-unicode-scalar`, `reader-character-supplementary-scalar`, `reader-character-invalid-surrogate`, `reader-character-invalid-surrogate-end`, `reader-character-invalid-out-of-range`, `reader-character-malformed-hex`, `reader-character-folded-name`, `reader-symbol-case-directive`, `reader-no-fold-case-directive`, `reader-string-line-continuation`, `reader-number-radix-prefix`, `reader-numeric-exactness-before-radix`, `reader-numeric-radix-before-exactness`, `reader-numeric-duplicate-prefix-error`, `reader-numeric-like-peculiar-identifier`, `reader-numeric-nondecimal-rectangular`, `reader-dotted-list`, `reader-vector-literal`, `reader-abbreviation-forms`, `reader-comments-and-datum-comments`, `reader-escaped-identifier`, `reader-datum-label-cycle` | First-pass reader datums, including owned Unicode-scalar characters, supplementary external forms, malformed and invalid scalar rejection, escaped identifiers, string continuations, radix/exactness prefix ordering, nondecimal complex syntax, malformed numeric-prefix rejection, bytevector range errors, and shared and circular datum labels, are implemented and fixture-loaded with Consent Scheme's reader. |
...
| Exceptions | `with-exception-handler`, `guard`, `raise`, `raise-continuable`, `error` | `implemented` | `exceptions-guard-raise`, `exceptions-guard-primitive-error`, `exceptions-guard-primitive-error-object`, `exceptions-error-object-accessors`, `exceptions-raise-continuable`, `exceptions-dynamic-wind-unwind` | Error objects remain inspectable through `error-object?`, `error-object-message`, and `error-object-irritants`; dynamic-wind cleanup is exercised while exceptions unwind. Conditions raised by primitive procedures (for example `(car 5)`) are catchable by `guard` and satisfy `error-object?`. |
...
| Core data types | Booleans, numbers, characters, strings, symbols, pairs, lists, vectors, bytevectors, procedures, records, ports, EOF objects | `policy-gated` | `core-data-character-scalar-model`, `core-data-character-invalid-surrogate`, `core-data-character-invalid-scalar-matrix`, `core-data-character-string-port-boundaries`, `core-data-character-comparison-matrix`, `core-data-vector-ref`, `core-data-record-type`, `core-data-owned-symbol-identity`, `core-data-owned-pair-shared-mutation`, `core-data-owned-vector-shared-mutation`, `core-data-owned-string-shared-mutation`, `core-data-owned-bytevector-shared-mutation`, `core-data-eof-object`, `standard-library-bytevector-ports` | Characters, symbols, and compound datums have owned identity; alias-visible mutation is fixture-covered for pairs, strings, vectors, and bytevectors alongside scalar boundaries, records, EOF objects, parameters, and in-memory ports. Host-managed ports remain policy-gated. |
| Numeric tower | Exact and inexact integers, rationals, reals, complex numbers, arithmetic, comparison, conversions | `implemented` | `numeric-exact-rational-arithmetic`, `numeric-exact-integer-growth`, `numeric-rational-reduction-growth`, `numeric-exactness-conversions`, `numeric-exactness-conversion-boundary`, `numeric-mixed-exact-inexact-ordering`, `numeric-radix-string-conversions`, `numeric-rationalize-tolerance`, `numeric-exact-integer-sqrt-large`, `numeric-complex-rectangular-arithmetic`, `numeric-complex-division`, `numeric-complex-operation-matrix`, `numeric-complex-polar-geometry`, `numeric-inexact-special-values`, `numeric-inexact-edge-arithmetic`, `numeric-binary64-rounding-boundaries`, `numeric-polar-special-values`, `numeric-transcendental-accuracy-matrix`, `numeric-transcendental-domain-boundaries`, `numeric-compiled-hot-path-smoke`, `numeric-canonical-rendering`, `standard-library-inexact-transcendentals` | The portable runtime owns exact limbs, normalized rationals, finite binary64 arithmetic and conversion, exact-dyadic mixed comparison, and round-tripping rendering. Exact growth, rational reduction, radix I/O, conversion, rationalization, large exact integer square roots, rectangular and polar complex arithmetic, inexact special-value canonicalization, predicates, tolerance-bounded transcendental accuracy and domains, and compiled numeric hot paths are fixture-covered. The representation and retained transcendental-accelerator contract is documented in [Self-Hostable Numeric Backend](numeric-backend.md). |
| Equivalence | `eq?`, `eqv?`, `equal?` across standard datums | `implemented` | `core-equivalence-owned-value-categories`, `core-equivalence-opaque-location-tags`, `core-equivalence-membership-and-association`, `core-data-circular-equal`, `core-data-owned-symbol-identity`, `core-data-owned-pair-cycle-mutation-equality`, `core-data-owned-vector-cycle-mutation-equality` | Scheme-visible identity is Consent-owned across scalar, compound, record, procedure, and port values. Circular graphs terminate, and membership and association procedures use their named predicates. Fixed implementation choices are documented in [Consent Value Equivalence](value-equivalence.md). |
| Ports and I/O datums | Textual and binary ports, input/output procedures, reader and writer round trips | `policy-gated` | `standard-library-write-display`, `standard-library-write-shared`, `standard-library-write-shared-after-alias-mutation`, `standard-library-write-owned-cycles-after-mutation`, `standard-library-write-simple`, `standard-library-write-circular`, `standard-library-write-string-range-newline`, `standard-library-read-string-port`, `standard-library-read-write-roundtrip`, `standard-library-owned-symbol-roundtrip`, `standard-library-bytevector-ports`, `standard-library-read-bytevector-partial`, `standard-library-current-output-port` | In-memory string and bytevector ports support focused read/write round trips, including aliases and cycles after owned mutation, owned-symbol identity, partial binary reads, range writes, and newline output without host access. Current/default ports and host-backed ports are policy-gated. |

## Standard Libraries

| Library | Status | Representative fixtures | Notes |
| --- | --- | --- | --- |
| `(scheme base)` | `policy-gated` | `primitive-procedure-call`, `derived-let-expression`, `multiple-values-call-with-values`, `core-data-character-scalar-model`, `core-data-character-invalid-scalar-matrix`, `core-data-character-string-port-boundaries`, `core-data-character-comparison-matrix`, `numeric-radix-string-conversions`, `numeric-rationalize-tolerance`, `standard-library-read-bytevector-partial`, `standard-library-write-string-range-newline`, `standard-library-base-features-utf8`, `standard-library-base-utf8-unicode-scalars`, `standard-library-base-utf8-range-slices`, `standard-library-base-utf8-rejects-split-range-start`, `standard-library-base-utf8-rejects-split-range-end`, `standard-library-base-utf8-rejects-stray-continuation`, `standard-library-base-utf8-rejects-truncated-two-byte`, `standard-library-base-utf8-rejects-truncated-three-byte`, `standard-library-base-utf8-rejects-truncated-four-byte`, `standard-library-base-utf8-rejects-bad-two-byte-continuation`, `standard-library-base-utf8-rejects-bad-three-byte-continuation`, `standard-library-base-utf8-rejects-bad-four-byte-continuation`, `standard-library-base-utf8-rejects-overlong`, `standard-library-base-utf8-rejects-overlong-three-byte`, `standard-library-base-utf8-rejects-overlong-four-byte`, `standard-library-base-utf8-rejects-surrogate`, `standard-library-base-utf8-rejects-surrogate-end`, `standard-library-base-utf8-rejects-above-maximum`, `standard-library-base-utf8-rejects-high-leading-byte`, `standard-library-base-utf8-rejects-invalid-leading-byte` | Core syntax, pure procedures, owned character conversion and ordering, strict UTF-8 branch boundaries, malformed-input rejection and multibyte range slicing, radix numeric conversion, rationalization, and in-memory textual/binary port helpers are implemented. Current/default ports and host-facing I/O remain policy-gated. |
...
| `(scheme char)` | `implemented` | `standard-library-char-upcase`, `standard-library-char-foldcase`, `standard-library-char-owned-unicode-profile`, `standard-library-char-owned-classification-tables`, `standard-library-char-owned-digit-whitespace-tables`, `standard-library-char-owned-case-mappings`, `standard-library-char-owned-case-tables`, `standard-library-char-comparison-matrix` | Character predicates, every finite bootstrap classification and mapping table, digit values, whitespace, and all variadic case-insensitive character and string comparisons use the deterministic [bootstrap Unicode profile](character-model.md). Scalars outside its tables remain valid, classify false, and case-map to themselves; issue #727 tracks generated full Unicode tables. |
...
| `(scheme inexact)` | `implemented` | `numeric-inexact-special-values`, `numeric-inexact-edge-arithmetic`, `numeric-binary64-rounding-boundaries`, `standard-library-inexact-transcendentals` | Predicates and representative real-valued transcendental procedures normalize through canonical Consent values. General transcendental host math remains a documented temporary accelerator under the [numeric backend contract](numeric-backend.md). |
...
| `(scheme read)` | `policy-gated` | `standard-library-read-string-port`, `standard-library-read-write-roundtrip`, `standard-library-owned-symbol-roundtrip` | `read` is implemented for explicit in-memory string input ports, uses the Consent Scheme reader, and preserves owned-symbol identity. Default current input and host ports remain policy-gated. |
...
| `(scheme write)` | `policy-gated` | `standard-library-write-display`, `standard-library-write-shared`, `standard-library-write-shared-after-alias-mutation`, `standard-library-write-owned-cycles-after-mutation`, `standard-library-write-simple`, `standard-library-write-circular`, `standard-library-write-string-range-newline`, `standard-library-read-write-roundtrip`, `standard-library-owned-symbol-roundtrip`, `standard-library-current-output-port` | In-memory string output with `display`, `write`, `write-shared`, `write-simple`, aliases and cycles after owned mutation, owned-symbol round trips, range writes, and newline output is implemented; default current output and host-backed ports remain policy-gated. |
