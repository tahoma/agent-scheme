...
| Numeric tower | Exact and inexact integers, rationals, reals, complex numbers, arithmetic, comparison, conversions | `implemented` | `numeric-exact-rational-arithmetic`, `numeric-exact-integer-growth`, `numeric-rational-reduction-growth`, `numeric-exactness-conversions`, `numeric-exactness-conversion-boundary`, `numeric-mixed-exact-inexact-ordering`, `numeric-radix-string-conversions`, `numeric-rationalize-tolerance`, `numeric-exact-integer-sqrt-large`, `numeric-complex-rectangular-arithmetic`, `numeric-complex-division`, `numeric-inexact-special-values`, `numeric-inexact-edge-arithmetic`, `numeric-polar-special-values`, `numeric-canonical-rendering`, `standard-library-inexact-transcendentals` | The portable runtime owns exact limbs, normalized rationals, finite binary64 arithmetic and conversion, exact-dyadic mixed comparison, and round-tripping rendering. Exact growth, rational reduction, radix I/O, conversion, rationalization, large exact integer square roots, rectangular complex arithmetic, polar and inexact special-value canonicalization, predicates, and representative real-valued transcendental procedures are fixture-covered. The representation and retained transcendental-accelerator contract is documented in [Self-Hostable Numeric Backend](numeric-backend.md). |
...
| `(scheme inexact)` | `implemented` | `numeric-inexact-special-values`, `numeric-inexact-edge-arithmetic`, `standard-library-inexact-transcendentals` | Predicates and representative real-valued transcendental procedures normalize through canonical Consent values. General transcendental host math remains a documented temporary accelerator under the [numeric backend contract](numeric-backend.md). |
