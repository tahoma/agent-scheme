...
| Core data types | Booleans, numbers, characters, strings, symbols, pairs, lists, vectors, bytevectors, procedures, records, ports, EOF objects | `policy-gated` | `core-data-character-scalar-model`, `core-data-character-invalid-surrogate`, `core-data-character-invalid-scalar-matrix`, `core-data-character-string-port-boundaries`, `core-data-character-comparison-matrix`, `core-data-vector-ref`, `core-data-record-type`, `core-data-owned-symbol-identity`, `core-data-eof-object`, `standard-library-bytevector-ports` | Characters and symbols have owned identity; the full scalar validity boundary, variadic scalar ordering, BMP through maximum-scalar string and port crossings, records, EOF objects, parameters, and in-memory string and bytevector ports are fixture-covered. Host-managed ports remain policy-gated. |
...
| Equivalence | `eq?`, `eqv?`, `equal?` across standard datums | `implemented` | `core-data-vector-ref`, `core-data-circular-equal`, `core-data-owned-symbol-identity` | Circular pair and vector comparisons terminate; reader, conversion, and macro-introduced symbols share portable identity. |
| Ports and I/O datums | Textual and binary ports, input/output procedures, reader and writer round trips | `policy-gated` | `standard-library-write-display`, `standard-library-write-shared`, `standard-library-write-simple`, `standard-library-write-circular`, `standard-library-write-string-range-newline`, `standard-library-read-string-port`, `standard-library-read-write-roundtrip`, `standard-library-owned-symbol-roundtrip`, `standard-library-bytevector-ports`, `standard-library-read-bytevector-partial`, `standard-library-current-output-port` | In-memory string and bytevector ports support focused read/write round trips, including owned-symbol identity, partial binary reads, range writes, and newline output without host access. Current/default ports and host-backed ports are policy-gated. |
