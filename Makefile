...
CONSENT_EMACS_TOOLS_TEST_SELECTOR ?= (or "consent-ci.*" "consent-compile.*" "consent-control-loop-doc.*" "consent-debugger.*" "consent-diagnostics.*" "consent-diff.*" "consent-docstring-metadata-doc.*" "consent-feature-reflection-doc.*" "consent-job.*" "consent-native-cli-daemon-doc.*" "consent-reflect.*" "consent-repl.*" "consent-skill.*" "consent-smoke.*" "consent-vcs.*")
