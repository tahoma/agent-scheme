...
(ert-deftest agent-scheme-runtime-test-version-components-are-canonical ()
  ...
  (should (equal (agent-scheme-version-components) '(0 14 11)))
  (should (equal (agent-scheme-value->external (agent-scheme-version))
                 "(agent-scheme-version 0 14 11)"))
  (should (equal (agent-scheme-version-string) "0.14.11")))
        ...
        (unless (equal file canonical)
          (with-temp-buffer
            ...
            (when (re-search-forward
                   "\\(agent-scheme-version 0 14 11\\|0\\.14\\.11\\|'(0 14 11\\|(list 0 14 11\\)"
                   nil t)
              (push file matches))))))
    (should-not matches)))
