...
(defun consent-vcs-test--write-file (path text)
  "Write TEXT to PATH, creating parent directories."
  (make-directory (file-name-directory path) t)
  (with-temp-file path
    (insert text)))

(defun consent-vcs-test--git (root &rest arguments)
...
(defun consent-vcs-test--cached-files (root)
  "Return staged file names in ROOT."
  (split-string
   (consent-vcs-test--git root "diff" "--cached" "--name-only")
   "\n"
   t))

(defun consent-vcs-test--current-branch (root)
  "Return the current branch name in ROOT."
  (string-trim
   (consent-vcs-test--git root "branch" "--show-current")))

(defun consent-vcs-test--make-git-repo ()
  "Return a temporary Git repository with one modified tracked file."
  (unless (executable-find "git")
    (ert-skip "git executable is not available"))
  (let* ((root (file-name-as-directory
                (make-temp-file "consent-emacs-vcs-" t)))
         (tracked (expand-file-name "src/main.scm" root))
         (untracked (expand-file-name "scratch.scm" root)))
    (consent-vcs-test--git root "init" "-q" "-b" "main")
    (consent-vcs-test--write-file tracked "(define value 1)\n")
    (consent-vcs-test--git root "add" "src/main.scm")
    (consent-vcs-test--git
     root
     "-c" "user.name=Consent Scheme Tests"
     "-c" "user.email=consent-tests@example.invalid"
     "commit" "-q" "-m" "initial commit")
    (consent-vcs-test--write-file tracked "(define value 2)\n")
    (consent-vcs-test--write-file untracked "(define scratch #t)\n")
    root))

(defmacro consent-vcs-test--with-project-root (root &rest body)
...
(ert-deftest consent-vcs-test-status-parser-covers-clean-state ()
...
(ert-deftest consent-vcs-test-status-parser-covers-worktree-states ()
...
(ert-deftest consent-vcs-test-raw-diff-parser-covers-file-summaries ()
...
(ert-deftest consent-vcs-test-local-mutation-requires-policy-grant ()
...
(ert-deftest consent-vcs-test-emacs-vcs-mutation-imports-separate-bindings ()
...
(ert-deftest consent-vcs-test-emacs-vcs-stage-denies-without-vcs-authority ()
  ...
  (let ((consent-policy-category-actions
        ...
        (root (consent-vcs-test--make-git-repo)))
    (unwind-protect
        (consent-vcs-test--with-project-root root
          ...
          (should (equal (consent-vcs-test--cached-files root) nil))
          (should
...
(ert-deftest consent-vcs-test-emacs-vcs-stage-denies-by-host-policy ()
  "Host policy denial stops a VCS mutation before Git changes the index."
  (let ((consent-policy-category-actions
         (consent-vcs-test--actions '((vcs-mutation . deny))))
        (root (consent-vcs-test--make-git-repo)))
    (unwind-protect
        (consent-vcs-test--with-project-root root
          (consent-audit-clear)
...
(ert-deftest consent-vcs-test-emacs-vcs-stage-with-grant-mutates-index ()
            ...
            (consent-vcs-test--external
             "(import (scheme base)
              ...
              (list
               ...
               (vcs-outcome-status outcome))")
            "(ok ok)"))
          (should (equal (consent-vcs-test--cached-files root)
                         '("src/main.scm")))
          (should
           (consent-vcs-test--audit-entry-matching
            "(event vcs-capability-audit)"
...
(ert-deftest consent-vcs-test-emacs-vcs-commit-branch-and-switch-with-grant ()
  ...
  (let ((consent-policy-category-actions
    ...
    (unwind-protect
        (progn
          (consent-vcs-test--git
           root "config" "user.name" "Consent Scheme Tests")
          (consent-vcs-test--git
           root "config" "user.email" "consent-tests@example.invalid")
          (consent-vcs-test--with-project-root root
            (consent-audit-clear)
            (should
             (equal
            ...
            (should (equal (consent-vcs-test--current-branch root)
                           "feature/topic"))
...
(ert-deftest consent-vcs-test-emacs-vcs-maps-git-status-diff-and-log ()
  ...
  (let ((root (consent-vcs-test--make-git-repo)))
    (unwind-protect
        (consent-vcs-test--with-project-root root
          (consent-audit-clear)
          (let ((result
                 (consent-vcs-test--result-external
                  "(import (scheme base) (emacs vcs))
                   (vcs-yield (vcs-status '()))
                   'ok")))
            (should (consent-vcs-test--contains-p result "(status ok)"))
            (should
             (consent-vcs-test--contains-p
              result
