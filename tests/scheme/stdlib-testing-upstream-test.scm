;;; Adapted upstream SRFI 64 meta-test suite.
;; SPDX-License-Identifier: MIT
;; SPDX-FileCopyrightText: 2005, 2006 Per Bothner
;; SPDX-FileCopyrightText: 2026 Tahoma Toelkes
;;;
;;; Upstream: https://github.com/scheme-requests-for-implementation/srfi-64
;;; Revision: 2d6a9fc514050f75802ef0003cfbf4846602a3ee
;;; Source blob: aab90d1130d50c1dee945ef209c3fc83841355bf
;;; Local patches: add R7RS imports; install a fail-closed current runner;
;;; preserve the upstream meta-tests otherwise.

(import (scheme base)
        (scheme write)
        (stdlib testing))

;; Return a runner that makes unexpected failures and passes fatal to the host.
(define (make-self-test-runner)
  (let ((runner (test-runner-null)))
    (test-runner-on-bad-count!
     runner
     (lambda (runner count expected)
       (error "SRFI 64 self-test count mismatch" count expected)))
    (test-runner-on-bad-end-name!
     runner
     (lambda (runner begin end)
       (error "SRFI 64 self-test group-name mismatch" begin end)))
    (test-runner-on-test-end!
     runner
     (lambda (runner)
       (if (memq (test-result-kind runner) '(fail xpass))
           (begin
             (display "SRFI 64 SELF-TEST ")
             (display (if (eq? (test-result-kind runner) 'xpass)
                          "XPASS "
                          "FAIL "))
             (write (test-runner-test-name runner))
             (newline)))))
    (test-runner-on-final!
     runner
     (lambda (runner)
       (if (or (> (test-runner-fail-count runner) 0)
               (> (test-runner-xpass-count runner) 0))
           (error "SRFI 64 self-test failed"
                  (test-runner-fail-count runner)
                  (test-runner-xpass-count runner))
           (begin
             (display "SRFI 64 upstream meta-tests passed")
             (newline)))))
    runner))

(test-runner-current (make-self-test-runner))
...
;; Run THUNK and capture every test's group path and name.
...
;; Run THUNK while collecting test names in the runner auxiliary value.
