;;; Adapted upstream SRFI 214 flexvector tests.
;; SPDX-License-Identifier: MIT
;; SPDX-FileCopyrightText: 2020-2021 Adam Nelson
;; SPDX-FileCopyrightText: 2026 Tahoma Toelkes
;;;
;;; Upstream: https://github.com/scheme-requests-for-implementation/srfi-214
;;; Revision: 37dc6dd7adebc19e2c58dde618f159ebee741d7e
;;; Source: implementation/tests.scm
...
;; Fail-closed runner for the otherwise unchanged upstream assertions.
...
(test-equal "make-flexvector" #(a a a)
...
(test-equal "flexvector->string" "abc"
  (flexvector->string (flexvector #\a #\b #\c)))

;; Mutable source list used by the upstream mock generator.
(define genlist '(a b c))
(define (mock-generator)
  "Return the next GENLIST value or the end-of-file object."
  (if (pair? genlist)
...
(let ((fv (flexvector 10 20 30)))
  ...
  (test-equal "reverse-flexvector->list" '(30 20 10)
...
;; Evaluate EXPR with NAME bound to a flexvector copy of VEC.
...
(test-equal "flexvector-append! multiple" '#(foo bar baz qux quux)
  (mutate-as x '#(foo bar)
    (flexvector-append! x (flexvector 'baz 'qux) (flexvector 'quux))))
...
(test-equal "flexvector-reverse-copy! bounded" #(1 40 30 20 5)
  (mutate-as fv '#(1 2 3 4 5)
    (flexvector-reverse-copy!
      fv 1 (flexvector 10 20 30 40 50) 1 4)))
...
(if (or (> (test-runner-fail-count upstream-runner) 0)
    ...
    (error "SRFI 214 upstream tests failed"
