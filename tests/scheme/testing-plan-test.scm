  ...
  (let ((programs (testing-plan-programs project-plan))
        ...
        (compiled-shards
         (map (lambda (name) (testing-plan-files project-plan name))
              '(compiled-runtime
                ...
                compiled-random
                compiled-property))))
    (test-equal "compiled project shard program count" 46
    ...
    (test-assert "compiled project shard includes runtime manifest smoke"
                 (member "tests/scheme/consent-manifest-smoke-test.scm"
                         compiled-files))
    (test-assert "compiled project shard includes runtime storage"
                 (member "tests/scheme/consent-runtime-storage-test.scm"
                         compiled-files))
    (test-equal "programs admitted to compiled self-host" 46
                (program-count-with-tag programs 'compiled))
    (test-equal "ordinary full-suite programs" 66
