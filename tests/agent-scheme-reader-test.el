...
(ert-deftest agent-scheme-reader-test-character-writer-canonical-forms ()
  ...
  (dolist (case '(("#\\space" . "#\\space")
    ...
    (let* ((source (car case))
           (expected (cdr case))
           (external (agent-scheme-reader-test--external source)))
      (should (equal external expected))
      (should (equal (agent-scheme-reader-test--external external)
                     expected)))))
